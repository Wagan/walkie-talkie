...
#if defined(UWB_CHIP_DW3000)
/* Публичная обёртка над аппаратным сбросом RSTn — тот же механизм, что команда dwreset. Нужна
 * платформенному слою (dw3000_port.c): uwbinit начинает со сброса, иначе повторный вызов не
 * возвращает чип в IDLE_RC (см. REPORT_lab09_init_fixes §3). Объявлена в dw3000_port.h. */
void Uwb_HardReset(void) { uwb_hard_reset(); }
#endif
