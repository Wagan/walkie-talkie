/**
  ...
  *  - режим SPI 0 (CPOL=0, CPHA=0); старший бит вперёд; CSn низкий всю транзакцию;
  *  - потолок SPICLK в состоянии INIT (после сброса) <= 3 МГц (UM Table 1).
  ******************************************************************************
  */
 ...
 * ВНИМАНИЕ: отдельного DW1000/DWM1000 Datasheet в репозитории НЕТ (есть только
 * DW1000 User Manual v2.17). Оттуда: после снятия сброса кристалл стабилизируется и
 * модуль входит в INIT примерно за 4 мс (Table 1), затем INIT->IDLE за +5 мкс; в
 * состоянии INIT уже допустим SPI-доступ на частоте <= 3 МГц.
 *   - время ожидания после отпускания RSTn берём > 4 мс (из Table 1) с запасом;
 *   - минимальной длительности НИЗКОГО импульса на RSTn в доступной документации
 *     нет (там RSTn описан как выход; физика вывода -> Datasheet, которого у нас нет),
 *     поэтому держим низким с большим запасом. Обе величины легко изменить здесь. */
#define DW_RST_LOW_MS    2u    /* удержание RSTn в низком уровне (запас; точный минимум в доке отсутствует) */
